...
The service resolves the KEK to hand to CEI:
  * `kms_name` set, real provider — service generates a random
    KEK, wraps it via the named registry slot, persists the
    envelope, and uses the plaintext KEK as `index_key` on the
    core call.  CEI generates the DEK and wraps it under that KEK
    internally.
  * `kms_name` set, `provider: none` — SDK-supplied `index_key`
    is the KEK; envelope persisted with empty `wrapped_kek`.
  * `kms_name` omitted — SDK-supplied `index_key` is the KEK;
    envelope persisted with `provider: none`.
