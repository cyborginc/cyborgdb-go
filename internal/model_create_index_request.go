...
type CreateIndexRequest struct {
	...
	// Name of a kms.registry entry in the service YAML. When supplied, the service wraps a fresh DEK via that KMS and persists the envelope; the SDK doesn't need to provide index_key unless the entry is provider=none.
	KmsName NullableString `json:"kms_name,omitempty"`
	// 32-byte encryption key as hex string. Required when kms_name is omitted, or when kms_name points at a provider=none registry entry.
