...
type BinaryQueryRequest struct {
	...
	// 32-byte encryption key as hex string. Optional when the index is KMS-backed (the service resolves the KEK via the index's KMSBlob); required when the index is `provider: none`.
